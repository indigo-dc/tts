...
handle_call({credential_add, UserId, ServiceId, CredState}, _From, #state{con=Con}=State) ->
...
handle_call({credential_remove, UserId, ServiceId, CredState}, _From, #state{con=Con}=State) ->
...
credential_add(UserId,ServiceId,CredState,Con) -> 
    ...
    esqlite3:q("INSERT INTO tts_cred VALUES( ?1, ?2, ?3);",[UserId,
                                                                 ServiceId,
                                                                 CredState], Con), 
...
credential_get(UserId,Con) -> 
    esqlite3:q("SELECT serviceid, credstate FROM tts_cred WHERE  userid IS ?",[UserId], Con). 
...
-define(TABLES, [
                 {cred,<<"tts_cred">>, <<"CREATE TABLE tts_cred(userid TEXT, serviceid TEXT, credstate TEXT)">>}
                ]).

create_tables_if_needed(Con) ->
    ok = esqlite3:exec("begin;", Con), 
    CreateTable = fun({_,Name,Create}, _) ->
                          Select = << <<"SELECT * FROM ">>/binary, Name/binary >>,
