...
| email_on_error_to | Whom to send emails in case of plugin issues. Only if emails are enabled and mail on plugin errors are enabled (see server settings) | 'noone', 'admin', 'developer' or 'admin_devel' | no (admin_devel) |
